    ...
    for r in range(N):         # r is the current row
        for c in range(N):     # c is the current column
            ...
            if r > 0 and r < N - 1 and c > 0 and c < N - 1:
                total = (current[r-1][c-1] + current[r-1][c+1] + current[r+1][c-1] + current[r+1][c+1] + current[r][c-1] + current[r][c+1] + current[r-1][c] + current[r+1][c])
            ...
            # middle bottom row (5 neighbors)
                ...
                total = (current[r][c-1] + current[r-1][c-1] + current[r-1][c] + current[r-1][c+1] + current[r][c+1])

            # middle left column (5 neighbors)
            
            elif c == 0 and r > 0 and r < N - 1:
                total = (current[r-1][c] + current[r-1][c+1] + current[r][c+1] + current[r+1][c+1] + current[r+1][c])
...
def main():
    ...
    N = 0
    while N < 10 or N > 500:
        N = int(input("Enter grid size (10-500): "))
